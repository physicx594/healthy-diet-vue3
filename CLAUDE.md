...
- **樣式**：Tailwind CSS v4（utility-first），自訂主題定義在 `src/assets/tailwind.config.css`
  - 色彩：Primary (#2D6A4F)、Primary Light (#52B788)、Primary Dark (#1B4332)、Secondary (#D4A373)、Accent (#FAEDCD)、Contrast (#C1553B)
  - 自訂動畫與 z-index 層級皆在此檔案內設定
  - **禁止使用 arbitrary values**（如 `text-[14px]`、`w-[200px]`），需要時在 `tailwind.config.css` 的 `@theme {}` 新增 design token
